return {
    ...
    commands = {
        ...
        setup = {
            ...
            action = function(_options, _, _, _)
                ...
                if _noOptions or _options.app then
                    am.execute_extension("__eth/download-binaries.lua", { contextFailExitCode = EXIT_SETUP_ERROR })
                end
                ...
                if _noOptions or _options.configure then
                    am.app.render()
                    am.execute_extension("__etho/configure.lua", { contextFailExitCode = EXIT_APP_CONFIGURE_ERROR })
                end
                log_success("Ether-1 node setup complete.")
            end
        },
        start = {
            description = "ami 'start' sub command",
            summary = "Starts the ETH node",
            action = "__eth/start.lua",
            contextFailExitCode = EXIT_APP_START_ERROR
        },
        stop = {
            description = "ami 'stop' sub command",
            summary = "Stops the ETHO node",
        ...
        about = {
            ...
            action = function(_options, _, _, _)
                local _ok, _aboutFile = fs.safe_read_file("__etho/about.hjson")
